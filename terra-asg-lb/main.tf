...
provider "aws" {
        region = "us-east-1"
        shared_config_file      = ["/opt/terrafrom/asg-and-lb/.aws/config"]
        shared_credentials_file = ["/opt/terrafrom/asg-and-lb/.aws/credentials"]
        profile = "PROFILE"
}
...
# (for the private subnet to access internet - eg. ec2 instances downloading softwares from in$resource "aws_nat_gateway" "sh_nat_for_private_subnet" {
...
resource "aws_launch_template" "sh_ec2_launch_templ" {
  ...
  image_id      = "ami-0f409bae3775dc8e5" # To note: AMI is specific for each region
  ...
  tag_specifications {
    ...
    tags = {
      Name = "nick-instance" # Name for the EC2 instances
    }
  }
}
